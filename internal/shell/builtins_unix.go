...
// builtins_unix.go — Unix platform-specific built-in commands (Linux + macOS + BSD).
package shell
...
// rlimitDefs contains the cross-platform Unix subset; builtins_linux.go appends
// Linux-only entries (RLIMIT_NICE, RLIMIT_SIGPENDING, RLIMIT_MSGQUEUE, …) via init().
...
func registerPlatformBuiltins(r *Registry) {
	...
	registerLinuxBuiltins(r) // no-op on macOS/BSD; defined in builtins_linux.go / builtins_unix_other.go
}
